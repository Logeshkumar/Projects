...
void main(int argc, char *argv[]) 
{
  ...
  {
    if(argc > 1)
		f.setPath(Convert::ToStdStr(argv[1]));
  }
  ...
  std::cout << "\n  Files in C:\\Temp";
  if(f.firstFile(TEXT("D:\\ood\\Navigate (3)\\Navigate"),TEXT("*.*")))
  {
    if(!f.isDirectory())
      std::cout << "\n  " <<Convert::ToString( f.name());
    while(f.nextFile())
      if(!f.isDirectory())
        std::cout << "\n  " << Convert::ToString( f.name());
  }
  ...
  sd.insert(f);
  ss.insert(f);
  f.showData((stdOstream &)std::cout);
  while(f.nextFile()) {
    sn.insert(f);
    sl.insert(f);
    sd.insert(f);
