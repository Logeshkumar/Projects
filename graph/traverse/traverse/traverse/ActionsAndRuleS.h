...
public:
	void doAction(ITokCollection* pTc)
	{
	 ...
	 c.se temp;
	 /*static std::vector<std::string> vectemp1;
	  string kee = c.GetHfile();
	  string key = c.GetKey();
	  if(vectemp1.size()==0 && temp!="none"&& temp!=kee && temp!=key)
	      {
		  vectemp1.push_back(temp);
	      }
      vector<std::string>::iterator it;
	  it = find (vectemp1.begin(), vectemp1.end(), temp);
	  if(temp!="none"&& it==vectemp1.end()&& temp!=kee&& temp!=key)
	      {     
	      vectemp1.push_back(temp);
	      }
	  c.SetVector(vectemp1);*/
  }
