...
//----< set path to return to on call to RestorePath() >---------------
  static std::vector<stdStr> hfiles;
   	 static std::vector<stdStr> cfiles;
...
// The gethvector function gets the .h files in a vector.
	
std::vector<stdStr> Directory::gethvector()
{
	return hfiles;
}
// The getcppvector function gets the .cpp files in a vector.
std::vector<stdStr> Directory::getcppvector()
{
	return cfiles;
}
...
// Hence from all the files, the .cpp files are stored in a vector
// and .h files are stored in a separate vector
  ...
  std::cout << std::endl; 
   for(size_t i=0; i<headerfiles.size(); ++i)
   {
	 ...
	 newString = ztring.substr(0,position);
	  std::string data = Convert::ToString(newString);
       std::transform(data.begin(), data.end(), data.begin(), ::toupper);
	   newString = Convert::ToStdStr(data);
	 hfiles.push_back(newString);
	}
   for(size_t i=0; i<cppfiles.size(); ++i)
   {
	 size_t position;
	 stdStr ztring = cppfiles[i];
	 stdStr cead = TEXT("."); 
	 position = ztring.find(cead);
	 stdStr nwString ;
	 
	 nwString = ztring.substr(0,position);
	  std::string data = Convert::ToString(nwString);
       std::transform(data.begin(), data.end(), data.begin(), ::toupper);
	   nwString = Convert::ToStdStr(data);
