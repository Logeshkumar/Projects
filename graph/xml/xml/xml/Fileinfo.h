...
/*
    ...
    - Had to replace every instance of the terms *first and *second
      ...
      Visual Studio 2010.
    ver 3.1 : 18 Jan 09
    - fixed text handling bug in fileInfo::firstFile(path,pattern) 
    ver 3.0 : 13 Jan 09
...
namespace Win32Tools
{
  ...
  public:
    ...
    // search
    bool firstFile(const stdStr& filePattern=TEXT("*.*"));
    ...
    // quick display
    void showData(stdOstream &out,stdChar ch=TEXT('\n'),int width=20) const;
  ...
  inline stdStr fileInfo::lastSetPath() { return _CurrPath; }

  //----< return file name >-------------------------------------

  inline stdStr fileInfo::name() const { return data.cFileName; }

  //----< less than operator >-----------------------------------

  inline bool fileInfo::operator<(const fileInfo &fd) const 
  {
    return (compare(data.cFileName,fd.data.cFileName) == -1);
  }
  ...
  {  
    return (compare(data.cFileName,fi.data.cFileName) == 0);
  }
