...
/*
    Maintenance History
    ===================
	ver 2.0 : 24 March 2011
	- Added the following functions to the xmlTran
    void funcPackage();
	void funch();
	void funcpp();
	void CreateXmlDoc();
	...
	- The CreateEmpDep() function xml meta data for the files which are present in the directory but its 
      referred in a file or included in a file basically a xml file with no dependency is created for the 
	  files refered but not in the directory.
    ver 1.3 : 22 Feb 06
...
class xmlRep {
  ...
  public:
    ...
    strVal format();
	void makeDirectory(std::string,xmlRep);
