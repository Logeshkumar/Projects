...
ConfigParseToConsole::ConfigParseToConsole() 
  : pToker(0), pSemi(0), pParser(0),dDetection(0), dDependencyStore(0),eEdge(0),eEdgeStore(0)
    {}
...
Parser* ConfigParseToConsole::Build()
{
  try
  {
	...
	eEdge = new Edge;
	eEdgeStore = new EdgeStore;
	eEdge->addAction(eEdgeStore);
	pParser->addRule(eEdge);
	//return pParser;
