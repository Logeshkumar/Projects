 ...
 * ===================
 * ver 2.1 : 28 Sep 10
 *   - corrected bugs discovered by Akshay Baviskar
 *   - modified CStoker.getTok()
 *   - changed CStoker.isNextTokenComment() to isComment(tok)
 *   - in CSemiExp changed returnComments property's set to make change in CStoker. 
 * ver 2.0 : 28 Sep 10
 *   - added boolean property returnComments to allow user to decide
 *   - changed display to more accurately convey what is collected, fixing a bug
 *     in comment handling.
 * ver 1.9 : 27 Sep 08
...
namespace CSsemi
{
  ...
  {
    CStoker.CToker toker = null;
    ArrayList semiExp = null;
    string currTok = "";
    string prevTok = "";
    bool Verbose_ = false;
    bool returnNewLines_ = true;
    bool displayNewLines_ = false;
    bool returnComments_ = true;
    bool displayComments_ = true;
    ...
    {
      get { return (string)semiExp[i]; }
      set { semiExp[i] = value;        }
    }
    //----< insert token - silently fails if index out of range >--------
    ...
    public void display()
    {
      Console.Write("\n----------------\n");
      int count = 0;
      foreach(string tok in this.semiExp)
      {
        if(tok == "\n" && (!displayNewLines_))
          continue;
        if (isComment(tok))
    ...
    {
      get { return returnNewLines_; }
      set { returnNewLines_ = value; }
    }
    //----< determines whether new lines are displayed >-----------------

    public bool displayNewLines
    {
      get { return displayNewLines_; }
      set { displayNewLines_ = value; }
    }
    //----< determines whether comments are displayed >------------------

    public bool returnComments
    {
      get { return returnComments_; }
      set { 
        returnComments_ = value;
        if (toker != null)
          toker.returnComments(returnComments_);
      }
    }
    //----< determines whether comments are returned >-------------------

    public bool displayComments
    {
      get { return displayComments_; }
      set { displayComments_ = value; }
    }
