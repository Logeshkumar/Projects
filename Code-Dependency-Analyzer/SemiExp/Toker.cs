 ...
 * ver 2.3 : 28 Sep 10
 *   - added member function returnComments(bool rtnComm) to provide a choice
 *   - modified getTok() and added isNextTokComment() to support that
 * ver 2.2 : 10 Jun 08
...
namespace CStoker
{
  ...
  class CToker
  {
    ...
    private ArrayList tokBuffer = null;   // intermediate token store
    private string lineRemainder;         // unprocessed line fragment
    private bool returnComments_ = true;
    ...
    //----< should toker return comments? >----------------------------

    public void returnComments(bool rtnComm)
    {
      returnComments_ = rtnComm;
    }
    //----< is next token in token buffer a comment? >-----------------

    private bool isComment(string tok)
    {
    ...
    public string getTok()
    {
      ...
      {
        tok = peekNextTok();
        if (!returnComments_ && isComment(tok))
        {
